...
1. it outputs JavaScript instead of JSON [by default](jsesc.md#json), enabling support for data structures like ES6 maps and sets;
2. it offers [many options](jsesc.md#api) to customize the output;
3. its output is ASCII-safe [by default](jsesc.md#minimal), thanks to its use of [escape sequences](https://mathiasbynens.be/notes/javascript-escapes) where needed.
...
* U+2028 `\u2028`
* U+2029 `\u2029`
* whatever symbol is being used for wrapping string literals \(based on [the `quotes` option](jsesc.md#quotes)\)
...
The `indentLevel` option takes a numeric value, and defaults to `0`. It represents the current indentation level, i.e. the number of times the value of [the `indent` option](jsesc.md#indent) is repeated.
...
The `lowercaseHex` option takes a boolean value \(`true` or `false`\), and defaults to `false` \(disabled\). When enabled, any alphabetical hexadecimal digits in escape sequences as well as any hexadecimal integer literals \(see [the `numbers` option](jsesc.md#numbers)\) in the output are in lowercase.
