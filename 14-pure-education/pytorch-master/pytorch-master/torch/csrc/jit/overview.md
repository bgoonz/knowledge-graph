...
* [JIT Technical Overview](overview.md#jit-technical-overview)
  * [Table of Contents](overview.md#table-of-contents)
* [Core Program Representation](overview.md#core-program-representation)
  * [Modules](overview.md#modules)
  * [Parameters](overview.md#parameters)
  * [Method](overview.md#method)
  * [FunctionSchema](overview.md#functionschema)
  * [Graph](overview.md#graph)
  * [Node](overview.md#node)
  * [Block](overview.md#block)
    * [If](overview.md#if)
    * [Loops](overview.md#loops)
  * [Value](overview.md#value)
  * [Type](overview.md#type)
* [Generating Programs](overview.md#generating-programs)
  * [Tracer](overview.md#tracer)
  * [Script](overview.md#script)
  * [Tree](overview.md#tree)
  * [Tree Views](overview.md#tree-views)
  * [frontend.py](overview.md#frontendpy)
  * [Lexer](overview.md#lexer)
  * [Tokens](overview.md#tokens)
  * [Parser](overview.md#parser)
  * [Compiler](overview.md#compiler)
  * [SugaredValue](overview.md#sugaredvalue)
  * [Resolver](overview.md#resolver)
  * [Environment](overview.md#environment)
  * [SSA Conversion](overview.md#convert_to_ssa)
  * [Python-Compiler Interaction](overview.md#python-compiler-interaction)
* [Executing Programs](overview.md#executing-programs)
  * [Evaluation Semantics](overview.md#evaluation-semantics)
  * [IValue](overview.md#ivalue)
  * [Operation](overview.md#operation)
  * [Operator](overview.md#operator)
  * [Interpreter](overview.md#interpreter)
  * [Graph Executor](overview.md#graph-executor)
  * [JIT Logging](overview.md#jit-logging)
  * [JIT Optimization Limitter](overview.md#jit-optimization-limitter)
  * [DifferentiableGraphOp](overview.md#differentiablegraphop)
  * [Interpreter](overview.md#interpreter-1)
  * [FusionGroup](overview.md#fusiongroup)
  * [Handling Mutability](overview.md#handling-mutability)
    * [Aliasing and mutation in the PyTorch API](overview.md#aliasing-and-mutation-in-the-pytorch-api)
    * [Aliasing and mutation annotations in FunctionSchema](overview.md#aliasing-and-mutation-annotations-in-functionschema)
    * [Alias Analysis in the IR](overview.md#alias-analysis-in-the-ir)
    * [Writing optimization passes with AliasDb](overview.md#writing-optimization-passes-with-aliasdb)
* [Profiling Programs](overview.md#profiling-programs)
* [Saving Programs](overview.md#saving-programs)
* [Testing Programs](overview.md#testing-programs)
  * [Test Autodiff](overview.md#test-autodiff)
  * [Python Printer](overview.md#python-printer)
* [Python Bindings](overview.md#python-bindings)

## Core Program Representation

### Modules \#\#

[api/module.h](https://github.com/bgoonz/Knowledge-Bank/tree/d157cab4a536be397d8f7d36c79f7d69d282500a/14-Pure-Education/pytorch-master/pytorch-master/torch/csrc/jit/api/module.h)
...
[aten/src/ATen/core/function\_schema.h](https://github.com/bgoonz/Knowledge-Bank/tree/d157cab4a536be397d8f7d36c79f7d69d282500a/14-Pure-Education/pytorch-master/pytorch-master/aten/src/ATen/core/function_schema.h)
...
[ir.h](https://github.com/bgoonz/Knowledge-Bank/tree/d157cab4a536be397d8f7d36c79f7d69d282500a/14-Pure-Education/pytorch-master/pytorch-master/torch/csrc/jit/ir/ir.h)

Graphs are the root of the intermediate representation \(IR\) used to define the implementation of TorchScript functions. If you are familiar with [LLVM](https://github.com/bgoonz/Knowledge-Bank/tree/d157cab4a536be397d8f7d36c79f7d69d282500a/14-Pure-Education/pytorch-master/pytorch-master/torch/csrc/jit/llvm.org), they are analogous to an `llvm::Function` object. A Graph is composed of Nodes, Blocks, and Values. Nodes are instructions \(e.g. do a matrix multiply\). Nodes are organized into Blocks of sequentially executed Nodes. Each Node produces a list of output Values, and also consumes a list of input Values. As an example, a user may write the following TorchScript code:
...
Because Graph owns all its Nodes, Values, and Blocks, these values are always passed around by raw pointer. Generally developers should not write code that holds Value, Node, or Block objects indefinitely without also holding a shared\_ptr to their owning Graph.
...
> _Code in the JIT should always assume the universe of valid Node kinds is open and subject to be expanded._
...
For if-statements \(`prim::If`\) the Blocks have no inputs, and the outputs are the new values of variables in the outer block whose values were altered in an if-statement. Example IR for an if-statement looks like:
...
[aten/src/ATen/core/jit\_type.h](https://github.com/bgoonz/Knowledge-Bank/tree/d157cab4a536be397d8f7d36c79f7d69d282500a/14-Pure-Education/pytorch-master/pytorch-master/aten/src/ATen/core/jit_type.h)
...
* TensorType - a tensor with optionally refined information. It may optional know its device, type, requires\_grad state, the number of dimensions.
...
[tracer.h](https://github.com/bgoonz/Knowledge-Bank/tree/d157cab4a536be397d8f7d36c79f7d69d282500a/14-Pure-Education/pytorch-master/pytorch-master/torch/csrc/jit/frontend/tracer.h) [tracer\_state.h](https://github.com/bgoonz/Knowledge-Bank/tree/d157cab4a536be397d8f7d36c79f7d69d282500a/14-Pure-Education/pytorch-master/pytorch-master/torch/csrc/jit/frontend/tracer_state.h)
...
As the trace runs, individual operators create Nodes in the Graph being traced to record what happens. This code is currently generated per operator in [tools/autograd/gen\_variable\_type.py](https://github.com/bgoonz/Knowledge-Bank/tree/d157cab4a536be397d8f7d36c79f7d69d282500a/14-Pure-Education/pytorch-master/pytorch-master/tools/autograd/gen_variable_type.py). It results in code that looks like the following:
...
> _When a trace calls a TorchScript function, that function is preserved as is, meaning that control-flow is preserved._ This makes it possible to "fix" tracing issues by writing the subset of the program that cannot be traced in script and having the trace invoke it.
...
The script frontend directly converts Python syntax into Modules. Like many compilers this happens in two phases. First, we generate an abstract syntax tree \(AST\), which is constructed out of Tree objects. The compiler \(misnamed, but that is the name of the file\) then does semantic analysis on the Tree and lowers it into a Module. We can generate Trees in two ways: \(1\) using frontend.py, which takes the Python AST and transliterates it into Tree objects, or \(2\) via the Lexer and Parser which parse python syntax directly. The Lexer/Parser path may seem redundant but it is crucially important. We need to define builtin functions \([script/builtin\_functions.cpp](https://github.com/bgoonz/Knowledge-Bank/tree/d157cab4a536be397d8f7d36c79f7d69d282500a/14-Pure-Education/pytorch-master/pytorch-master/torch/csrc/jit/script/builtin_functions.cpp)\) when Python is not linked. We allow users to load TorchScript programs directly from strings without Python \([api/include/torch/jit.h](https://github.com/bgoonz/Knowledge-Bank/tree/d157cab4a536be397d8f7d36c79f7d69d282500a/14-Pure-Education/pytorch-master/pytorch-master/torch/api/include/torch/jit.h)\). We also use this Python syntax as the serialization format for TorchScript, since it allows us to make changes to our IR without breaking backward compatibility. Furthermore, the Lexer is reused to implement the FunctionSchema parser, which turns FunctionSchema declarations from strings into FunctionSchema objects.

The following sections look into each the stages in the script frontend in detail.

### Tree \#\#

[frontend/tree.h](https://github.com/bgoonz/Knowledge-Bank/tree/d157cab4a536be397d8f7d36c79f7d69d282500a/14-Pure-Education/pytorch-master/pytorch-master/torch/csrc/jit/frontend/tree.h)
...
[frontend/tree\_views.h](https://github.com/bgoonz/Knowledge-Bank/tree/d157cab4a536be397d8f7d36c79f7d69d282500a/14-Pure-Education/pytorch-master/pytorch-master/torch/csrc/jit/frontend/tree_views.h)
...
[torch/jit/frontend.py](https://github.com/bgoonz/Knowledge-Bank/tree/d157cab4a536be397d8f7d36c79f7d69d282500a/14-Pure-Education/pytorch-master/pytorch-master/torch/jit/frontend.py)
...
> _We endeavor to keep most of the JIT code written in C++, because most of the JIT functionality still needs to work without Python installed._

So this code simply constructs the Tree, filtering out the AST nodes of Python that we do not support.

### Lexer \#\#

[frontend/lexer.h](https://github.com/bgoonz/Knowledge-Bank/tree/d157cab4a536be397d8f7d36c79f7d69d282500a/14-Pure-Education/pytorch-master/pytorch-master/torch/csrc/jit/frontend/lexer.h)
...
[frontend/parser.h](https://github.com/bgoonz/Knowledge-Bank/tree/d157cab4a536be397d8f7d36c79f7d69d282500a/14-Pure-Education/pytorch-master/pytorch-master/torch/csrc/jit/frontend/parser.h)
...
[frontend/ir\_emitter.h](https://github.com/bgoonz/Knowledge-Bank/tree/d157cab4a536be397d8f7d36c79f7d69d282500a/14-Pure-Education/pytorch-master/pytorch-master/torch/csrc/jit/frontend/ir_emitter.h)
...
[frontend/sugared\_value.h](https://github.com/bgoonz/Knowledge-Bank/tree/d157cab4a536be397d8f7d36c79f7d69d282500a/14-Pure-Education/pytorch-master/pytorch-master/torch/csrc/jit/frontend/sugared_value.h)
...
_The combination of SugaredValue and Resolver decouples the implementation of the IR emitter from the pybind11 Python bindings that enable its interaction with the Python state._
...
[frontend/convert\_to\_ssa.cpp](https://github.com/bgoonz/Knowledge-Bank/tree/d157cab4a536be397d8f7d36c79f7d69d282500a/14-Pure-Education/pytorch-master/pytorch-master/torch/csrc/jit/frontend/convert_to_ssa.cpp)

As explained in the  _Block_  section, the IR is represented in structured control flow composed of ifs & loops. This makes it easier to optimize and lower to other compilers which do not support unstructured control flow. We lower python control flow \(break, continue, return\) to this simplified form. We do closing over any variables in the environment, so we are able to convert all writes and reads from the environment directly to SSA form.

Conversion to SSA works in multiple parts.

* First, we add loads and stores to control flow operators \(ifs & loops\).
* Then we erase Break & Continue statements from the graph and replace them with `prim::LoopContinuation`. `prim::LoopContinuation` has the form `LoopContinuation(%loop_continue_condition, %loop_carried_vars)`. Break Statements have the continue condition set to false, and Continue statements inline the loop condition. %loop\_carried\_vars are the loop carried variables of the inner most loop that contains the Break or Continue statement, are added by inserting prim::Loads calls at the location of the statement.
...
* Finally, we remove `prim::LoopContinuation`s and `prim::ReturnStmt`s in the exit\_transform pass.

### Exit Transform \#\#

[frontend/exit\_transform.cpp](https://github.com/bgoonz/Knowledge-Bank/tree/d157cab4a536be397d8f7d36c79f7d69d282500a/14-Pure-Education/pytorch-master/pytorch-master/torch/csrc/jit/frontend/convert_to_ssa.cpp)
...
In the above example, the if statement will have one output, with the value on the false branch being `math.sqrt(i)`. In the true branch, insert and use `prim::Uninitialized`. These are values inserted by the compiler when it can prove the value will never be used. It can be introduced by exceptions, breaks, continues, and returns.
...
[python/script\_init.cpp](https://github.com/bgoonz/Knowledge-Bank/tree/d157cab4a536be397d8f7d36c79f7d69d282500a/14-Pure-Education/pytorch-master/pytorch-master/torch/csrc/jit/python/script_init.cpp)
...
It is important to remember that TorchScript uses these semantics for Tensors so not all computation on Tensor is pure. Individual Tensors may be _views_ of the same underlying data. Views are established by special view creating operations, such as indexing into a tensor:
...
* requires\_grad - whether the tensor is recording its gradient with autograd
...
[ivalue.h](https://github.com/bgoonz/Knowledge-Bank/tree/d157cab4a536be397d8f7d36c79f7d69d282500a/14-Pure-Education/pytorch-master/pytorch-master/torch/include/ATen/core/ivalue.h)
...
[runtime/operator.h](https://github.com/bgoonz/Knowledge-Bank/tree/d157cab4a536be397d8f7d36c79f7d69d282500a/14-Pure-Education/pytorch-master/pytorch-master/torch/csrc/jit/runtime/operator.h)
...
[runtime/interpreter.cpp](https://github.com/bgoonz/Knowledge-Bank/tree/d157cab4a536be397d8f7d36c79f7d69d282500a/14-Pure-Education/pytorch-master/pytorch-master/torch/csrc/jit/runtime/interpreter.cpp)
...
[runtime/graph\_executor.cpp](https://github.com/bgoonz/Knowledge-Bank/tree/d157cab4a536be397d8f7d36c79f7d69d282500a/14-Pure-Education/pytorch-master/pytorch-master/torch/csrc/jit/runtime/graph_executor.cpp)
...
_Pre-derivative Optimization_ On a code cache miss, we generate a new optimized Graph on the fly \(`compileSpec`\). It starts by creating a copy of the initial Graph and setting the input types to the specialized Tensor types observed in this specialization. TensorType inputs to the Graph will get refined with types that know the device, number of dimensions, and requires grad state.
...
* It propagates the input ranks, dtypes, devices, and requires\_grad information to the rest of the graph where possible.
...
[jit\_log.h](https://github.com/bgoonz/Knowledge-Bank/tree/d157cab4a536be397d8f7d36c79f7d69d282500a/14-Pure-Education/pytorch-master/pytorch-master/torch/csrc/jit/jit_log.h)
...
[jit\_opt\_limit.h](https://github.com/bgoonz/Knowledge-Bank/tree/d157cab4a536be397d8f7d36c79f7d69d282500a/14-Pure-Education/pytorch-master/pytorch-master/torch/csrc/jit/jit_opt_limit.h)
...
You can call opt limiter by calling a macro `JIT_OPT_ALLOWED`. It will return true if we haven't reached the optimization limit yet. Otherwise, it will return false. Typical usage:
...
Some operators will _mutate_ one or more of their operands in-place. These are typically denoted with a trailing underscore, or by taking an `out` argument as input:
...
[ir/alias\_analysis.h](https://github.com/bgoonz/Knowledge-Bank/tree/d157cab4a536be397d8f7d36c79f7d69d282500a/14-Pure-Education/pytorch-master/pytorch-master/torch/csrc/jit/ir/alias_analysis.h) An alias analysis pass consumes the per-operator aliasing information to construct a database of aliasing and mutation relationships in a graph, called `AliasDb`. This section focuses on the alias analysis pass; the public interface to `AliasDb` will be described later.
...
The last point demonstrates a key concept: _leaf elements uniquely describe memory locations_. Since a leaf element doesn't point to anything, the memory that backs it must have been freshly allocated by some op. Thus we can use leaf elements to represent disjoint memory locations.
...
[runtime/symbolic\_script.cpp](https://github.com/bgoonz/Knowledge-Bank/tree/d157cab4a536be397d8f7d36c79f7d69d282500a/14-Pure-Education/pytorch-master/pytorch-master/torch/csrc/jit/runtime/symbolic_script.cpp)

When differentiating a graph, each node that has a symbolic gradient will be included in a `prim::DifferentiableGraph`. We fall back to use autograd for the node if there isn't a gradient formula for it. Adding/updating symbolic gradient functions must be tested carefully as it's easy to get CI green by comparing autograd result with itself, but potentially cause autodiff support regression.

If your PR adds/updates a gradient formula for `torch`/`nn` functions, you **MUST** enable/update the corresponding tests in

* `torch` functions: `method_tests` in [common\_method\_tests.py](https://github.com/bgoonz/Knowledge-Bank/tree/d157cab4a536be397d8f7d36c79f7d69d282500a/14-Pure-Education/pytorch-master/pytorch-master/test/common_method_tests.py)
* `nn` functions: `nn_functional_tests` in [test\_jit.py](https://github.com/bgoonz/Knowledge-Bank/tree/d157cab4a536be397d8f7d36c79f7d69d282500a/14-Pure-Education/pytorch-master/pytorch-master/test/test_jit.py)
...
All nodes in `nonfusible_nodes` should show up in at least once in `prim::DifferentiableGraph` subgraphs. When fusion is enabled, all nodes in `fusible_nodes` should show up in one of `prim::FusionGroup` graphs attached to `prim::DifferentiableGraph`, otherwise they're checked as `nonfusible_nodes` as well. On the other hand, if `should_check_autodiff=False`, the graph can still have `prim::DifferentiableGraph` with other nodes, but not `nonfusible_nodes` and `fusible_nodes`.
...
Note that even for the same function, different tests could trigger different function schemas \(e.g `aten::add`\) while only a few of them have symbolic gradient formulas. You should only turn on autodiff check in tests who have symbolic gradient. If you are not sure, uncomment the debugging line in [runtime/symbolic\_script.cpp](https://github.com/bgoonz/Knowledge-Bank/tree/d157cab4a536be397d8f7d36c79f7d69d282500a/14-Pure-Education/pytorch-master/pytorch-master/torch/csrc/jit/runtime/symbolic_script.cpp) to check which function schema the test triggers.

### Python Printer

[serialization/python\_print.cpp](https://github.com/bgoonz/Knowledge-Bank/tree/d157cab4a536be397d8f7d36c79f7d69d282500a/14-Pure-Education/pytorch-master/pytorch-master/torch/csrc/jit/serialization/python_print.cpp) [serialization/import\_source.cpp](https://github.com/bgoonz/Knowledge-Bank/tree/d157cab4a536be397d8f7d36c79f7d69d282500a/14-Pure-Education/pytorch-master/pytorch-master/torch/csrc/jit/serialization/import_source.cpp)

The Python Printer takes a `Graph` and produces Python-like code that represents the same graph. Using some special values in [serialization/import\_source.cpp](https://github.com/bgoonz/Knowledge-Bank/tree/d157cab4a536be397d8f7d36c79f7d69d282500a/14-Pure-Education/pytorch-master/pytorch-master/torch/csrc/jit/serialization/import_source.cpp), this code can be read back in by the compiler to produce the same `Graph`. In Python a `ScriptModule`'s `code` property shows the Python Printed graph.
